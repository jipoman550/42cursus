...
### 1단계: 입력 처리 및 유효성 검사
- [ ] 문자열을 숫자로 파싱 (atoi 또는 ft_atoi 구현 및 사용)
- [ ] 숫자 범위 확인 (INT_MIN ~ INT_MAX 초과 여부 검사)
- [ ] 중복 숫자 검사
- [ ] 유효하지 않은 입력(비숫자, 빈 문자열 등)에 대한 예외 처리 및 "Error\n" 출력 ?? 터미널 입력 형식? 그러니까 argc, argv일 때 "", '', 그냥 쓰기 이런것도 알아보면 좋을 듯
- [ ] 인자가 없는 경우 아무것도 출력하지 않고 종료

### 2단계: 스택 자료구조 구현
- [ ] 연결 리스트 기반의 스택 A, B 구성 (노드 구조체, 스택 헤더 구조체 등 정의)
- [ ] 기본 스택 연산 함수 구현:
	- [ ] push(스택 맨 위에 요소 추가)
	- [ ] pop(스택 맨 위 요소 제거 및 반환)
	- [ ] peek(스택 맨 위 요소 참조)
	- [ ] 스택 크기 확인 (ft_lstsize 또는 유사 함수 활용)
	- [ ] 스택 비어있는지 확인
- [ ] 스택 메모리 해제 함수 (free_stack) 구현 (메모리 누수 방지)
...
- [ ] ra, rb, rr
- [ ] rra, rrb, rrr
- [ ] 각 명령어 실행 후 해당 명령어 문자열을 표준 출력에 출력하는 로직 포함 ?? 이것도 뭔지 모름
...
- [ ] 디버깅용 명령어 기록 및 stack 상태 출력 함수 추가
