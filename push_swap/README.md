...
- [v] 3개 이하 숫자: 조건 분기
- [v] 4~5개 숫자: 최소 명령어 조합
- [v] 100개 이상: radix sort 기반 정렬
- [v] 메인 정렬 함수 통합
- [v] 입력된 값이 이미 정렬된 경우 (입력이 1일 경우도 포함됨)

### 5단계. 명령어 출력 시스템
- [v] 수행된 명령어를 stdout에 한 줄씩 출력
- [v] 디버깅용 명령어 기록 및 stack 상태 출력 함수 추가

### 6단계. 테스트 및 최적화
- [v] 다양한 입력 시나리오에 대한 정확성 테스트 (checker_OS 사용)
- [v] 명령어 수 확인 및 벤치마크 목표 달성 여부 검증
- [v] 메모리 누수 검사: valgrind 및 leaks 도구 활용
- [v] Norm 규정 준수 여부 최종 확인 (norminette 사용)
...
프로그램 동작 (Mandatory)
- [v] 인자 없을 시 아무것도 출력 안 하고 종료
- [v] 모든 오류 상황에서 정확히 "Error\n" 출력:
	- 정수가 아닌 인자 (0 one 2 3)
	- 정수 범위 (INT_MIN ~ INT_MAX) 초과 인자
	- 중복된 인자
- [v] stack a가 오름차순으로 완벽하게 정렬되고, 가장 작은 수가 맨 위에 위치하는지 확인
- [v] 출력된 명령어 시퀀스가 실제 정렬을 수행하며 checker_OS 프로그램으로 "OK"를 받는지 확인
- [v] 벤치마크 목표 달성 여부:
	- 100개 숫자: 700회 미만 (최소 1100회 미만)
	- 500개 숫자: 5500회 미만 (최소 8500회 미만)

코드 품질 및 규정 (Norm & Memory)
- [v] 모든 .c, .h 파일이 Norm 규정을 완벽히 준수하는지 (norminette 통과)
- [v] valgrind 및 leaks 검사를 통해 메모리 누수가 전혀 없는지 확인
- [v] 세그멘테이션 폴트, 버스 에러 등 예기치 않은 프로그램 종료 없음
- [v] 전역 변수 사용 금지 규정 준수
- [v] 허용되지 않은 외부 함수 사용 여부 재확인 (허용: read, write, malloc, free, exit, ft_printf 또는 동등한 직접 구현 함수)

Makefile
- [v] $(NAME), all, clean, fclean, re 규칙 모두 정상 작동
- [v] cc -Wall -Wextra -Werror 플래그 적용 확인
- [v] 불필요한 재링크 발생 안 함
- [x] (보너스 구현 시) bonus 규칙 정상 작동

---

## 📝 메모

- [v] 디버깅용 `print_stack()` 함수는 최종 제출 전 제거
- [v] 실행파일 이름은 `push_swap`
