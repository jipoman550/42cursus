...
#### 7. **const 멤버 함수 (const Member Functions)**

- **정의**: 함수 선언 뒤에 `const` 키워드를 추가하여, 해당 함수가 객체의 상태를 변경하지 않음을 명시함.
- **보장 내용**:
  - 함수 내부에서 멤버 변수에 값을 대입하는 모든 행위를 금지함.
  - 다른 비-const(non-const) 멤버 함수를 호출하는 것도 금지함.
- **필수성**:
  - **데이터 보호**: 단순 조회 함수(Getter)에서 의도치 않은 값 변경 방지.
  - **상수 객체 지원**: `const`로 선언된 객체는 오직 `const` 멤버 함수만 호출 가능하므로, 필수적인 설계 규약임.

| 구분 | 일반 멤버 함수 | `const` 멤버 함수 |
| :--- | :--- | :--- |
| **멤버 변수 수정** | 가능 | **불가능 (컴파일 에러)** |
| **this 포인터 타입** | `Account* const` | `const Account* const` |
| **상수 객체에서 호출** | 불가능 | **가능** |


#### 8. **정적 유틸리티 함수: `_displayTimestamp`**

-   **역할**: 표준 로그 형식에 맞춰 현재 시스템 시간을 `[YYYYMMDD_HHMMSS]` 형태로 출력함.
-   **정적(static) 설계 이유**:
    1.  **독립성**: 개별 객체의 상태(`this`)를 참조할 필요가 없는 독립적인 로직임.
    2.  **호환성**: 클래스 내부의 다른 정적 멤버 함수(예: `displayAccountsInfos`)에서도 호출이 가능해야 하므로 `static`으로 선언됨.
    3.  **캡슐화**: 클래스 외부의 전역 함수로 만들 수도 있지만, `Account` 클래스 전용 로그 포맷이므로 클래스 내부에 `private static`으로 가두어 관리함.


#### 9. **반복자 (Iterator)**
- **정의**: 컨테이너 내부의 원소를 순회하고 접근할 수 있게 해주는 포인터 객체.
- **장점**: 컨테이너의 내부 구조(배열, 리스트 등)를 몰라도 동일한 방식으로 데이터를 다룰 수 있음 (**일관된 인터페이스**).
- **주요 함수**:
    - `container.begin()`: 첫 번째 원소의 위치 반환.
    - `container.end()`: 마지막 원소 다음의 허구적 위치(끝) 반환.
- **주요 연산**:
    - `*` (Dereference): 가리키는 원소의 갑에 접근.
    - `++` (Increment): 다음 원소로 이동.
    - `->` (Arrow): 가리키는 객체의 멤버에 접근.
