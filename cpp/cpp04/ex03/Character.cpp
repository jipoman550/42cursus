...
{
	std::cout << "Character: Constructor called [" << this->name << "]" << std::endl;
	for (int i = 0; i < 4; i++)
	{
		this->slots[i] = NULL;
	}
...
Character::~Character()
{
	...
	std::cout << "Character: Destructor called [" << this->name << "]" << std::endl;
}
...
void Character::equip(AMateria *m)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		{
			this->slots[i] = m;
			std::cout << this->name << ": equip [" << m->getType() << "] at slot " << i << std::endl;
			return ;
		}
	}
...
void Character::unequip(int idx)
{
	...
	std::cout << this->name << ": unequip [" << this->slots[idx]->getType() << "] from slot " << idx << std::endl;
