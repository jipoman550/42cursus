...
Dog::Dog()
{
    ...
    std::cout << "Dog: 기본 생성자 호출 (brain 할당됨)" << std::endl;
}
...
Dog::Dog(const Dog &other) : Animal(other)
{
    ...
    std::cout << "Dog: 복사 생성자 호출 (brain 깊은 복사)" << std::endl;
}

// ── 대입 연산자: 기존 brain 해제 후 새 brain 깊은 복사 ──
Dog &Dog::operator=(const Dog &other)
{
    std::cout << "Dog: 대입 연산자 호출 (brain 깊은 복사)" << std::endl;
    if (this != &other)
    {
...
Dog::~Dog()
{
    ...
    std::cout << "Dog: 소멸자 호출 (brain 해제됨)" << std::endl;
}
