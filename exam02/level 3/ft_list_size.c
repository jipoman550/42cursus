...
// ??free해주는 동작을 하지 않으면, ft_list_size.c를 실행하고 나면 정말로 내 pc의 memory에 남아있어서 메모리 누수가 나는 거임? 그렇다면 이 .c 파일을 실행하면 계속 남아 있는다면 어떻게 없앨 수 있음? 컴퓨터 운영체제가 알아서 없애주나? -> 운영체제에서 알아서 없애준다네.